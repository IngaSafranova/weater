...
app.post("/weather", function (req, res) {
  ...
  const config = { responseType: "json", withCredentials: false };
  ...
  axios(url, config).then(async (response) => {
    ...
    const response_1 = await axios(
      `
         https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&units=metric&appid=${process.env.API_KEY}`,
      config
    );
